...
<div class="col-lg-8">
  <form method="POST" action="{{ route('roleuser.update', $roleuser->id) }}" class="mb-5" enctype="multipart/form-data">
  @method('put')
  @csrf
  <div class="mb-3">
    <label for="name_role" class="form-label @error('name_role') is-invalid @enderror">Role</label>
    <input type="text" name="name_role" class="form-control" id="name_role" required autofocus value="{{ old('name_role', $roleuser->name_role) }}">
    @error('name_role')
